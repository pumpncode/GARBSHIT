...
extern PRECISION vec2 fish;


extern Image fishTexture;
...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
    ...
    // take the pixel color from the fish Texture

    // Position of a pixel within the sprite
	vec2 uv = (((texture_coords)*(image_details)) - texture_details.xy*texture_details.ba)/texture_details.ba;

    vec4 Ptex = Texel(fishTexture, uv);
    ...
    if (fish.g > 0.0 || fish.g < 0.0) {
         hsl.y = 0.0;
         hsl.z *= (1 - adjusted_uv.x*(cos(fish.r*fac)));
         hsl.z *= (1 - adjusted_uv.y*(cos(fish.r*fac)));
    }
